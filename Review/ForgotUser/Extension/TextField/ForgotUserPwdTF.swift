...
@available(iOS 13.0, *)
extension ForgotUserPwdVC: UITextFieldDelegate {
    
    //휴대폰 번호 작성 시 이벤트
    @objc func textFieldDidChange(textField: UITextField) {
        if textField == self.phoneNumberTF {
            if textField.text!.count > 0 {
                self.phoneNumberGL.isHidden = true
            }else {
                self.phoneNumberGL.isHidden = false
            }
        }else {
            if textField.text!.count > 0 {
                self.userIdGL.isHidden = true
            }else {
                self.userIdGL.isHidden = false
            }
        }
